...
// - Tool results are normalized to { text, files } for Chubby Cat' prompt loop.
...
export class McpRemoteManager {
  constructor({ clientName = 'chubby-cat', clientVersion = '0.0.0' } = {}) {
  ...
  isEnabled(config) {
    ...
    // Support both legacy single server and multi-server configs
    if (Array.isArray(config?.mcpServers) && config.mcpServers.length > 0) {
      return enabled && config.mcpServers.some(s => s && s.url && s.url.trim() && s.enabled !== false);
    }
    return !!(enabled && config.mcpServerUrl);
  }

  // Get all active servers from config
  getActiveServers(config) {
    if (!config) return [];
    if (Array.isArray(config.mcpServers) && config.mcpServers.length > 0) {
      return config.mcpServers.filter(s => s && s.url && s.url.trim() && s.enabled !== false);
    }
    // Legacy single server support
    if (config.mcpServerUrl) {
      return [{
        id: config.mcpServerId || null,
        name: '',
        transport: config.mcpTransport || 'sse',
        url: config.mcpServerUrl,
        enabled: true,
        toolMode: config.mcpToolMode || 'all',
  ...
  async _sendRpc(method, params, options = {}) {
    if (this.transport === 'streamable-http') {
      return await this._sendRpcStreamableHttp(method, params, options.isInitialize || false);
    }
  ...
  async _sendRpcStreamableHttp(method, params, isInitialize = false) {
    ...
    // Include session ID for all requests except initialize
    if (!isInitialize && this.mcpSessionId) {
      headers['mcp-session-id'] = this.mcpSessionId;
    }
  ...
  async buildToolsPreamble(config) {
    // Multi-server support: process all active servers
    ...
    for (const server of activeServers) {
      try {
        // Create a config for this specific server
        const serverConfig = {
          enableMcpTools: true,
          mcpTransport: server.transport || 'sse',
          mcpServerUrl: server.url,
          mcpToolMode: server.toolMode || 'all',
          mcpEnabledTools: server.enabledTools || []
        };

        const tools = await this.listTools(serverConfig);
        if (!tools || tools.length === 0) continue;

        const mode = server.toolMode === 'selected' ? 'selected' : 'all';
        const enabledToolNames = Array.isArray(server.enabledTools) ? server.enabledTools : [];
        const enabledSet = mode === 'selected' ? new Set(enabledToolNames) : null;
        ...
        // Add server header if multiple servers
        if (activeServers.length > 1) {
          const serverName = server.name || server.url || 'MCP Server';
        ...
        console.warn(`Failed to fetch tools from server ${server.url}:`, e.message);
        lines.push(`[Error fetching tools from ${server.name || server.url}: ${e.message}]`);
        lines.push('');
      }
    }
  ...
  // Find which server provides a specific tool (for multi-server support)
  async findToolServer(config, toolName) {
    const activeServers = this.getActiveServers(config);
    ...
    for (const server of activeServers) {
      try {
        ...
        const mode = server.toolMode === 'selected' ? 'selected' : 'all';
        const enabledSet = mode === 'selected'
          ? new Set(Array.isArray(server.enabledTools) ? server.enabledTools : [])
          : null;

        const hasThisTool = tools.some(t => {
          if (t.name !== toolName) return false;
          if (mode === 'selected' && !enabledSet.has(t.name)) return false;
          return true;
        });
        ...
        console.warn(`Error checking tool on server ${server.url}:`, e.message);
      }
    }
  ...
  // Call a tool, finding the appropriate server in multi-server mode
  async callToolMulti(config, toolName, args) {
    const activeServers = this.getActiveServers(config);

    // If only one server, use simple call
    if (activeServers.length <= 1) {
      return this.callTool(config, toolName, args);
    }

    // Find which server has this tool
    const found = await this.findToolServer(config, toolName);
    if (!found) {
      throw new Error(`Tool '${toolName}' not found on any active MCP server`);
    }

    return this.callTool(found.serverConfig, toolName, args);
  }
}

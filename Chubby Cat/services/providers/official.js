 ...
 */
export async function sendOfficialMessage(prompt, systemInstruction, history, apiKey, modelName, thinkingLevel, files, signal, onUpdate, baseUrl) {
    ...
    // Use custom baseUrl or default
    let effectiveBaseUrl = (baseUrl || '').trim();
    const isCustomBaseUrl = !!effectiveBaseUrl;
    if (!effectiveBaseUrl) {
    ...
    // Build URL - for custom base URLs, we also add key as query param for compatibility
    const url = `${effectiveBaseUrl}/v1beta/models/${targetModel}:streamGenerateContent?alt=sse&key=${apiKey}`;

    // Build headers - always include x-goog-api-key header for broader compatibility
    // Some proxies/services prefer header-based auth over URL params
    const headers = {

...
export function appendMessage(container, text, role, attachment = null, thoughts = null) {
    ...
    if (currentText || currentThoughts || role === 'ai') {
        ...
        // --- Add Regenerate Button (AI messages only) ---
        if (role === 'ai') {
            const regenerateBtn = document.createElement('button');
            regenerateBtn.className = 'regenerate-btn';
            regenerateBtn.title = t('regenerate');

            const regenerateIcon = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/></svg>';
            const loadingIcon = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>';

            regenerateBtn.innerHTML = regenerateIcon;

            // Store icons for state management
            regenerateBtn.dataset.iconDefault = regenerateIcon;
    ...
    return {
        ...
        // Enable regenerate button (if this is an AI message)
        enableRegenerate: () => {
            const btn = div.querySelector('.regenerate-btn');
            if (btn) {
                btn.disabled = false;
                btn.classList.remove('loading');
                btn.innerHTML = btn.dataset.iconDefault || btn.innerHTML;
            }
        },
        // Disable regenerate button (if this is an AI message)
        disableRegenerate: () => {
            const btn = div.querySelector('.regenerate-btn');
            if (btn) {
                btn.disabled = true;
                btn.classList.add('loading');
                btn.innerHTML = btn.dataset.iconLoading || btn.innerHTML;
            }
        }
    };
}
